...
long long seg_func(long long x, long long y){
    return x+y;
}
void solve(){
    int n,q; cin>>n>>q;
    ...
    vector<long long> diff(n,0LL);
    SegmentTree<long long> sg(diff,0LL, seg_func);
    while(q--){
        int t; cin>>t;
        if(t==1){
            int l,r; long long diff_val;
            cin>>l>>r>>diff_val;
            long long lval = sg.query(l-1,l-1);
            long long rval = sg.query(r,r);
            sg.update(l-1, lval + diff_val);
            sg.update(r, rval - diff_val);
        }else{
            int qp; cin>>qp; --qp;
            cout<<v[qp] + sg.query(0,qp)<<endl;
        }
    }
}
